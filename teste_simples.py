...
def testar_api():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                "   Critérios:"
                f" termo_busca={resultado.get('criterios', {}).get('termo_busca')},"
                f" numero_processo={resultado.get('criterios', {}).get('numero_processo')},"
                f" classe_processual={resultado.get('criterios', {}).get('classe_processual')}"
            )

            resultados = resultado.get("processos", [])
            ...
            if resultados:
                ...
                for i, item in enumerate(resultados, 1):
                    ...
                    print(f"🔢 Número: {item.get('numeroProcesso', 'N/A')}")

                    if item.get("orgaoJulgador"):
                        print(f"🏛️  Órgão: {item.get('orgaoJulgador')}")
                    ...
                    if item.get("dataJulgamento"):
                        print(
                            "📅 Data Julgamento:"
                            f" {_formatar_data(item.get('dataJulgamento'))}"
                        )

                    if item.get("dataPublicacao"):
                        print(
                            "📰 Data Publicação:"
                            f" {_formatar_data(item.get('dataPublicacao'))}"
                        )

                    if item.get("ementa"):
                        ementa = item.get("ementa", "")[:300]
                        print("\n📝 Ementa (preview):")
                        print(f"   {ementa}...")
